...
def _worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        try:
            ...
            if cmd == 'step':
                observation, reward, done, action, info = env.step(data)
                if done:
                    # save final observation where user can get it, then reset
                    info['terminal_observation'] = observation
                    observation = env.reset()
                remote.send((observation, reward, done, action, info))
            elif cmd == 'reset':
                observation = env.reset()
...
class SubprocVecEnv(VecEnv):
    ...
    def step_wait(self):
        ...
        self.waiting = False
        obs, rews, dones, actions, infos = zip(*results)
        return _flatten_obs(obs, self.observation_space), np.stack(rews), np.stack(dones), np.stack(actions), infos
